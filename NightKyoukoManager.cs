// TODO:
// 本 NightKyoukoManager.cs 是 KyoukoManager.cs 的夜间版，是一个临时方案
// 目前仅用于测试夜间场景的 Kyouko 联机方案
// 采用的 API 也较为先进
// 后续需要考虑使用此版 API 方案重写 KyoukoManager.cs 并合并本文件逻辑
// 在有完整的 API 调用方案后应考虑合并入 KyoukoManager.cs
...
public class NightKyoukoManager
{
    private static NightKyoukoManager _instance;
    private static readonly object _lock = new object();
    
    public const string KYOUKO_ID = "NightKyouko";
    ...
    private static readonly string LOG_TAG = "[NightKyoukoManager.cs]";

    public static NightKyoukoManager Instance
    {
        get
        {
            if (_instance == null)
            {
                lock (_lock)
                {
                    _instance ??= new NightKyoukoManager();
                }
            }
            return _instance;
        }
    ...
    public static void SpawnNightKyouko()
    {
        Common.SceneDirector.Instance.SpawnCharacter(Common.SceneDirector.Identity.Special, 14, new Vector2(1, 1), "NightKyouko");
    ...
    public static void IgnoreCollisionWithSelf()
    {
        Physics2D.IgnoreCollision(
            Common.SceneDirector.Instance.characterCollection[KYOUKO_ID].cl2d,
